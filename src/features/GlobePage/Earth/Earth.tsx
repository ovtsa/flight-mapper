...
export default function Earth(props: EarthProps) {
    ...
    useFrame(() => {
        if(mesh && mesh.current && props.globeAnimating && !freezePassiveAnimation.current) {
    ...
    useEffect(() => {
        ...
        } else if (mesh && mesh.current) {
            rotateGlobeToPosition(0, mesh.current.rotation.y);
        }
    }, [props.coordinates, props.globeAnimating]);

    const rotateGlobeToPosition = (targetRadianX: number, targetRadianY: number, secondsOfAnimation = 1.5, framesPerSecond = 100) => {
